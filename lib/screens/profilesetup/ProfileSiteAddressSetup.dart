...
class ProfileSiteAddressSetup extends StatefulWidget {
  @override
  _ProfileSiteAddressSetupState createState() => _ProfileSiteAddressSetupState();
}

class _ProfileSiteAddressSetupState extends State<ProfileSiteAddressSetup> {
  ...
  void getBasicDetails() async {
    ...
    if (houseNo.isEmpty || address.isEmpty || city.isEmpty || state.isEmpty) {
      ...
      await users.doc(firebaseAuth.currentUser.uid).collection("siteAddress").doc("site").set(userSiteAddress).whenComplete((){

        animatedLoader.removeDialog();

        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => HomeWithBN()),
        );
      });
